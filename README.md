...
- 项目级记忆通过 `PAI.md` / `.codecli/PAI.md` 启动自动注入，适合提交到仓库的团队共享规则；`PAI.local.md` / `.codecli/PAI.local.md` 只做本地覆盖
...
- 配置持久化到 `~/.codecli/config.json`，API Key 可从配置、环境变量或 `.env` 读取
...
- 新增 `com.codecli.mcp` 模块，支持 stdio 子进程 server 与 Streamable HTTP 远程 server
- 启动时读取 `~/.codecli/mcp.json` 与 `.codecli/mcp.json`，项目级配置按 server 名覆盖用户级配置
...
- `~/.codecli/mcp.json` 不存在时会自动创建默认 chrome-devtools 配置；项目级 `.codecli/mcp.json` 仍可按 server 名覆盖
...
- `~/.codecli/mcp.json` 不存在时启动自动创建模板，默认使用 `--isolated=true` 临时浏览器 profile
...
- shared 模式下 `close_page` 只能关闭 CodeCLI 自己创建的 tab；无法证明是 CodeCLI 创建的 tab 会被策略层拒绝
...
- 三层加载位置（按优先级，后者整体覆盖同名 skill）：jar 内置 < 用户级 `~/.codecli/skills/<name>/` < 项目级 `<project>/.codecli/skills/<name>/`
- 启动期把启用 skill 的 `name` + `description` 注入三处 Agent 系统提示词索引段（启用上限 20 个，索引段 ≤ 4KB）
- 内置工具 `load_skill(name)`：LLM 在 system prompt 看到匹配 description 时主动调用，CodeCLI 把 SKILL.md 正文（5KB 截断）写入 `SkillContextBuffer`，下一轮 user message 自动前置注入
...
- 启用状态持久化：`~/.codecli/skills.json` 的 `disabled` 列表，默认全启用
- 与 HITL 协同：Skill 内调用 `execute_command` 等危险工具仍走既有 HITL 审批，沿用 `execute_command` 工具维度全放行；不给 Skill 单独审批维度

设计意图：从「写工具」演进到「打包专家手册」。当工具堆成山（CodeCLI 当前内置 9 个 + MCP 60+ 工具），用 Skill 给 LLM 一份按场景展开的"专家手册"，比往 system prompt 里塞更多规则更可扩展。
...
| 形态 | 启用方式 | 视觉风格 |
|---|---|---|
| **inline 流式 TUI**（默认） | 直接运行 / `CODECLI_RENDERER=inline` | Claude Code / Qoder 风格：π 主题彩色开屏、主屏直出、transcript 当前位置的 `* ` 输入提示、JLine `Status` 托管的底部 dock（YOLO/HITL、MCP、Skill、model、ctx、token、cwd 等关键字段带克制彩色高亮；ctx 是当前上下文估算，in/out/cache 是调用统计）、右侧输入提示、行内可折叠工具块（`Read 3 files (ctrl+o to expand)`）、行内 git diff、HITL 单字符 `[y/n/a/s/m]` 提示 |
| **lanterna 全屏 TUI** | `CODECLI_RENDERER=lanterna`（或兼容旧 `CODECLI_TUI=true`） | v16 三栏全屏：文件树 + 对话流 + 状态栏 + 底部输入栏，HITL 模态弹窗 |
| **plain 兜底** | `CODECLI_RENDERER=plain` | 纯 println，无折叠 / 状态栏，等价 v15 行为 |
...
- 对话历史保存到 `~/.codecli/history/session_*.jsonl`
- 兼容旧设置：`CODECLI_TUI=true` 自动映射为 `CODECLI_RENDERER=lanterna`（已 deprecated）
- `CODECLI_NO_STATUSBAR=true` 在 inline 模式下禁用 JLine 底部 dock（不适合 ANSI 光标控制的终端）
...
- 配置：`CODECLI_LSP_ENABLED=false` 可关闭，`CODECLI_LSP_MAX_DIAGNOSTICS=20` 可调整注入上限
...
- 快照仓库使用 JGit 纯 Java 实现，默认位于 `~/.codecli/snapshots/<project_hash>/<worktree_hash>/.git`，不写用户项目 `.git`
...
- 配置：`CODECLI_SNAPSHOT_ENABLED=false` 可关闭，`CODECLI_SNAPSHOT_MAX=50`、`CODECLI_SNAPSHOT_EXCLUDES=...`、`CODECLI_SNAPSHOT_DIR=...` 可调整策略
...
- 支持用户级覆盖 `~/.codecli/prompts/...`，支持项目级覆盖 `.codecli/prompts/...`，项目级优先级最高
- 覆盖是整文件替换；`base.md` 和最终 prompt 必须包含 `## Language`
- Prompt 改动审计模板见 `docs/prompt-analysis-template.md`

### 第二十期：异步后台任务 + Runtime API（MVP）

- `DurableTaskManager` 使用 SQLite 持久化后台任务队列，默认位置 `~/.codecli/tasks/tasks.db`
- 任务生命周期：`enqueued -> running -> completed / failed / canceled`
- `/task`、`/task add <任务内容>`、`/task cancel <task_id>`、`/task log <task_id>` 提供 CLI 闭环
- Worker Pool 默认 2 个后台 worker，可通过 `CODECLI_TASK_WORKERS` 调整
- `java -jar target/codecli-1.0-SNAPSHOT.jar serve --http --port 8080` 启动 localhost Runtime API
- Runtime API 端点：`POST /v1/threads`、`POST /v1/threads/{id}/turns`、`GET /v1/threads/{id}/events`
- Runtime API 强制要求 `CODECLI_RUNTIME_API_KEY` 或 `-Dcodecli.runtime.api.key`
...
- 新增进程级入口：`codecli wechat setup`、`codecli wechat start`、`codecli wechat status`、`codecli wechat daemon start|stop|restart|status|logs`
- 新增交互式入口：在 CodeCLI 主界面输入 `/wechat` 可扫码绑定并在当前进程后台启动微信通道；`/wechat setup` 重新扫码绑定，`/wechat status` 查看状态，`/wechat stop` 停止通道
...
- 微信侧用户消息会回显到 CodeCLI 终端 transcript；CodeCLI 终端继续显示 thinking / 工具调用过程，微信侧只接收 assistant 正文。iLink 协议层仍是 `text_item.text` 文本消息，没有显式 Markdown parse mode；CodeCLI 会保留 ClawBot 稳定支持的 Markdown 子集（列表、引用、粗体、行内代码、真实代码块），把标题转成粗体标题、把表格转成移动端更稳的键值/列表，并过滤图片 Markdown / H5-H6 / 中文斜体等兼容性差的标记；非代码类 fenced block（流程说明、长中文箭头链）会解包并换行，避免微信侧出现横向滚动代码块。iLink 不提供真正 SSE 或改单条消息能力。
...
`com.codecli.policy` 包，作为 HITL 之外的辅助层（不是沙箱、不提供进程隔离）：
...
- `AuditLog` 结构化审计：危险工具调用按天写 JSONL 到 `~/.codecli/audit/`，含 `outcome (allow|deny|error)` 与 `approver (hitl|policy|none)`；`revert_turn` 也纳入危险工具链
- `write_file` 单文件 5MB 上限
- CLI 命令：`/policy` 查看安全策略状态、`/audit [N]` 看最近 N 条审计

**为什么不叫沙箱**：本地 Agent CLI（参考 Claude Code / Cursor / Aider）默认都不做容器/VM 沙箱——沙箱削弱 Agent 能力、给虚假安全感、体验更差。生产级 Agent 沙箱实际是 microVM-level（Devin / Modal / Anthropic Computer Use 用 Firecracker / gVisor）。CodeCLI 的安全模型是 **HITL + 路径校验 + 命令快速拒绝 + 审计**，不是隔离。
...
- 📋 结构化审计：危险工具调用按天写一行 JSONL 到 `~/.codecli/audit/`，可通过 `/audit [N]` 查看
...
也可以在 CodeCLI 内用命令写入 `~/.codecli/config.json`，不会覆盖 Kimi 配置：
...
长期记忆默认保存在用户目录下的 `~/.codecli/memory/long_term_memory.json`。
...
- `~/.codecli/PAI.md`：用户级稳定偏好，所有项目可见。
- `PAI.md` / `.codecli/PAI.md`：项目级团队规则，建议提交到 git。
- `PAI.local.md` / `.codecli/PAI.local.md`：本地覆盖，适合个人调试约定，建议加入 `.gitignore`。
- `@relative/path.md`：在 `PAI.md` 中导入项目根内的相对文件；越靠后的文件越接近本地覆盖，优先级越高。

可用 `/init` 为当前项目生成一份短 `PAI.md`。该命令默认不覆盖已有文件；确认需要重建时使用 `/init --force`。
代码索引默认保存在 `~/.codecli/rag/codebase.db`。
调试日志默认滚动写入 `~/.codecli/logs/codecli.log`，旧日志会按保留天数和总容量自动清理。
...
```bash
# 指定记忆目录
java -Dcodecli.memory.dir=/tmp/codecli-memory -jar target/codecli-1.0-SNAPSHOT.jar

# 指定 RAG 索引目录
java -Dcodecli.rag.dir=/tmp/codecli-rag -jar target/codecli-1.0-SNAPSHOT.jar

# 指定日志目录与保留策略
java -Dcodecli.log.dir=/tmp/codecli-logs \
     -Dcodecli.log.level=DEBUG \
     -Dcodecli.log.maxHistory=3 \
     -Dcodecli.log.maxFileSize=5MB \
     -Dcodecli.log.totalSizeCap=20MB \
     -jar target/codecli-1.0-SNAPSHOT.jar
```

也可以放到 `.env` 或环境变量中：

```bash
CODECLI_LOG_LEVEL=DEBUG
CODECLI_LOG_DIR=/Users/yourname/.codecli/logs
CODECLI_LOG_MAX_HISTORY=7
CODECLI_LOG_MAX_FILE_SIZE=10MB
CODECLI_LOG_TOTAL_SIZE_CAP=100MB
```

### Agent Observability 与评测（Langfuse）

CodeCLI 可选接入 Langfuse 记录 Agent turn、LLM generation、工具调用、MCP、子 Agent、token、耗时和错误。默认关闭；启用后使用异步、有界队列上报，不影响 Agent 主流程。
...
Langfuse 负责 trace/span/generation 的组织、可视化和 Score 存储；任务成功、测试结果、文件 diff、工具轨迹和 Judge 分数由项目评测器计算。详细设计见 `docs/phase-24-langfuse-observability-eval.md`。

### 2. 可选：配置 MCP server

MCP 子系统默认开启。`~/.codecli/mcp.json` 不存在时，CodeCLI 会自动创建默认 chrome-devtools 配置：
...
需要继续接入其他 server 时，可编辑 `~/.codecli/mcp.json` 或项目内 `.codecli/mcp.json`：
...
`step_search` 是约定名称：如果项目 `.env`、用户 `~/.env` 或系统环境变量里存在 `STEP_API_KEY`，CodeCLI 会自动内置这个远程 MCP；上面的手写配置只用于覆盖默认地址或自定义鉴权。当前模型为 `step-3.7-flash*` 时，内置 `web_search` / `web_fetch` 会优先代理到该 MCP server。
...
```bash
# macOS
open -na "Google Chrome" --args --remote-debugging-port=9222 --user-data-dir=/tmp/codecli-chrome-profile

# Windows
start chrome.exe --remote-debugging-port=9222 --user-data-dir=%TEMP%\codecli-chrome-profile

# Linux
google-chrome --remote-debugging-port=9222 --user-data-dir=/tmp/codecli-chrome-profile
```

通常不需要用户预先切换；Agent 如果遇到登录页会自己调用 `browser_connect`。手工调试时也可以在 CodeCLI 内执行：
...
`/browser connect` 只在当前进程内把 `chrome-devtools` 切到 shared 模式，不会改写 `~/.codecli/mcp.json`。如果希望启动后默认 shared，可手动把 args 改为：
...
```bash
mvn clean compile exec:java -Dexec.mainClass="com.codecli.cli.Main"
```
...
同一轮模型返回多个工具调用时，CodeCLI 会并行执行这些工具；如果工具之间有依赖关系，模型应分多轮调用。
...
- `codecli wechat setup` - 绑定微信 iLink 通道，选择 workspace 并完成扫码确认
- `codecli wechat start` - 前台启动微信通道
- `codecli wechat status` - 查看绑定状态和 daemon pid
- `codecli wechat daemon start|stop|restart|status|logs` - 管理本机微信通道后台进程

交互式斜杠命令：

- `/wechat` - 扫码绑定并启动微信 iLink 通道；已绑定时直接启动
- `/wechat setup` - 重新扫码绑定并启动微信通道
- `/wechat status` - 查看当前 CodeCLI 进程内微信通道状态
- `/wechat stop` - 停止当前 CodeCLI 进程内微信通道
...
- `/export` - 导出当前 ReAct 会话对话记录为 Markdown（包含完整 system prompt），写入 `~/.codecli/exports/session-*.md`
