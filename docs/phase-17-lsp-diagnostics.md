...
```bash
# 默认开启
CODECLI_LSP_ENABLED=true

# 默认 20
CODECLI_LSP_MAX_DIAGNOSTICS=20
```

系统属性同样支持：

```bash
-Dcodecli.lsp.enabled=false
-Dcodecli.lsp.max.diagnostics=20
```
...
- 不解析 `execute_command` 里的手写补丁，也不支持 `edit_file` / `apply_patch` 工具，因为当前 CodeCLI 内置工具里还没有这两个工具。
