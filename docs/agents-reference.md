...
1. `~/.codecli/config.json` 中对应 provider 的 `apiKey`
...
| 数据 | 默认路径 | 覆盖方式 |
|------|----------|----------|
| 长期记忆 | `~/.codecli/memory/long_term_memory.json` | `-Dcodecli.memory.dir` |
| 项目级记忆 | `PAI.md` / `.codecli/PAI.md` / `PAI.local.md` | 用户级稳定偏好：`~/.codecli/PAI.md` |
| RAG 索引 | `~/.codecli/rag/codebase.db` | `-Dcodecli.rag.dir` |
| 审计日志 | `~/.codecli/audit/audit-YYYY-MM-DD.jsonl` | `CODECLI_AUDIT_DIR` / `-Dcodecli.audit.dir` |
| Side-Git 快照 | `~/.codecli/snapshots/<project_hash>/<worktree_hash>/.git` | `CODECLI_SNAPSHOT_DIR` / `-Dcodecli.snapshot.dir` |
| 后台任务 | `~/.codecli/tasks/tasks.db` | — |

### Snapshot Config

系统属性 > 环境变量 > 默认值：`codecli.snapshot.enabled`(true) / `codecli.snapshot.max`(50) / `codecli.snapshot.excludes`(.git,.codecli/snapshots,target,node_modules,dist,.idea,*.class,*.jar) / `codecli.snapshot.dir`(~/.codecli/snapshots)
...
系统属性 > 环境变量/.env > 默认值：`CODECLI_LOG_DIR`(~/.codecli/logs) / `CODECLI_LOG_LEVEL`(INFO) / `CODECLI_LOG_MAX_HISTORY`(7) / `CODECLI_LOG_MAX_FILE_SIZE`(10MB) / `CODECLI_LOG_TOTAL_SIZE_CAP`(100MB)

### ReAct/SubAgent Budget Config

系统属性 > 默认值：`codecli.react.token.budget`(Integer.MAX_VALUE) / `codecli.react.stagnation.window`(3) / `codecli.react.hard.max.iterations`(50)
...
系统属性 > 默认值：`codecli.llm.connect.timeout.seconds`(60) / `codecli.llm.read.timeout.seconds`(300) / `codecli.llm.write.timeout.seconds`(60) / `codecli.llm.call.timeout.seconds`(600)
...
1. 用户级：`~/.codecli/mcp.json`
2. 项目级：`.codecli/mcp.json`
...
- 加载顺序：`~/.codecli/PAI.md` → `PAI.md` → `.codecli/PAI.md` → `PAI.local.md` → `.codecli/PAI.local.md`
...
- shared 模式 close_page 只允许关闭 CodeCLI 创建的 tab

### Skill System

- 三层加载：jar 内置 < 用户级 ~/.codecli/skills/ < 项目级 .codecli/skills/
...
- 环境变量：`CODECLI_RENDERER=inline|lanterna|plain`
- `CODECLI_TUI=true`(旧) → lanterna + deprecation 提示
- `CODECLI_NO_STATUSBAR=true`：禁用底部状态栏
...
- `/export` 导出当前 ReAct conversationHistory 为 Markdown 到 `~/.codecli/exports/session-*.md`；包含完整 system prompt，便于检查 LLM 实际接收前的指令，命令不接受路径参数。
...
- `CODECLI_LSP_ENABLED=false` 关闭

### Git Side-History Snapshot (Phase 18)

- side-git 在 ~/.codecli/snapshots/ 维护独立仓库（JGit，不依赖系统 git）
...
- 覆盖优先级：jar 内置 < 用户级 ~/.codecli/prompts/ < 项目级 .codecli/prompts/
...
- XfyunMaaSClient：Qwen3.6-35B-A3B，默认 https://maas-api.cn-huabei-1.xf-yun.com/v2，OpenAI-compatible 讯飞星辰 MaaS；可用 `/config provider xfyun ...` 写入配置后 `/model xfyun` 切换。`model` 必须使用 MaaS 服务管控页展示的 modelId；微调模型可配置 `--lora-id <resourceId>`，作为 HTTP header `lora_id` 发出；该 provider 不发送 CodeCLI 内置 tools。
...
# CODECLI_SNAPSHOT_ENABLED=true
# CODECLI_SNAPSHOT_MAX=50
# CODECLI_SNAPSHOT_EXCLUDES=.git,.codecli/snapshots,target,node_modules,dist,.idea,*.class,*.jar
# CODECLI_SNAPSHOT_DIR=/Users/yourname/.codecli/snapshots
# CODECLI_TUI=true
# NO_TUI=true
```
