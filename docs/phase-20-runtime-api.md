...
- 生命周期：
  ...
  - `failed`
  - `canceled`
- Worker Pool：默认 2 个后台 worker，可用 `CODECLI_TASK_WORKERS` 或 `-Dcodecli.task.workers` 覆盖
...
实现位于 `src/main/java/com/codecli/runtime/api/`，使用 JDK 内置 `HttpServer`，不引入 Spring / Javalin。

启动：

```bash
CODECLI_RUNTIME_API_KEY=your_local_api_key \
...
- 仅监听 `127.0.0.1`
- 必须配置 `CODECLI_RUNTIME_API_KEY` 或 `-Dcodecli.runtime.api.key`
- 请求头支持：
  - `Authorization: Bearer <key>`
  - `X-CodeCLI-API-Key: <key>`
...
CODECLI_RUNTIME_API_KEY=test java -jar target/codecli-1.0-SNAPSHOT.jar serve --http --port 0
```
