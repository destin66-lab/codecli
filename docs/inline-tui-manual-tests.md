...
产物：`target/codecli-1.0-SNAPSHOT.jar`，所有用例都基于这个 jar 启动。
...
**预期**：
- Banner 显示简洁的 `CodeCLI v16.1.0` 开屏，不带右侧盒线边框
...
```bash
CODECLI_RENDERER=lanterna java -jar target/codecli-1.0-SNAPSHOT.jar
```
...
**预期**：
- stderr 出现 `⚠️ CODECLI_TUI=true 已废弃，请改用 CODECLI_RENDERER=lanterna`
...
```bash
CODECLI_RENDERER=plain java -jar target/codecli-1.0-SNAPSHOT.jar
```
...
```bash
NO_COLOR=1 java -jar target/codecli-1.0-SNAPSHOT.jar
```
...
```bash
CODECLI_RENDERER=weird java -jar target/codecli-1.0-SNAPSHOT.jar
```

**预期**：stderr `⚠️ 未识别的 CODECLI_RENDERER='weird'，回退到 inline`，正常进 inline 模式。
...
```bash
TERM=dumb java -jar target/codecli-1.0-SNAPSHOT.jar
```
...
```
*
 CodeCLI  idle  glm-5.1  ctx 0/200.0k  HITL OFF  MCP 4/4  Skill 2/2
...
**输入**：
```
把 ROADMAP.md 第一行改成 "# CodeCLI 路线图（v16.1 测试）"
```
...
│   [6] 配置文件: ~/.codecli/config.json (只读视图，编辑请用编辑器)
...
## 23. CODECLI_NO_STATUSBAR 禁用状态栏

```bash
CODECLI_NO_STATUSBAR=true java -jar target/codecli-1.0-SNAPSHOT.jar
```
...
- 旧 Windows cmd / PuTTY 等不适合 ANSI 光标控制的终端：用 `CODECLI_NO_STATUSBAR=true` 即可
