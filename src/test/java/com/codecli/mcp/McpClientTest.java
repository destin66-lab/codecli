...
class McpClientTest {
    ...
    void initializeTimeoutCanBeOverriddenBySystemProperty() {
        String previous = System.getProperty("codecli.mcp.initialize.timeout.seconds");
        try {
            System.setProperty("codecli.mcp.initialize.timeout.seconds", "17");
            assertEquals(17, McpClient.initializeTimeoutSeconds());
        } finally {
            if (previous == null) {
                System.clearProperty("codecli.mcp.initialize.timeout.seconds");
            } else {
                System.setProperty("codecli.mcp.initialize.timeout.seconds", previous);
            }
        }
    }
    ...
    void closeIsBestEffortAndDoesNotBlockOnServerSilence() throws Exception {
        // close 不再发 shutdown notification（server 卡死时会让 CodeCLI 退出阻塞）。
