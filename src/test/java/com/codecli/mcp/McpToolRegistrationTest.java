...
class McpToolRegistrationTest {
    ...
    private static void withAuditDir(Path tempDir, ThrowingRunnable body) throws Exception {
        String previous = System.getProperty("codecli.audit.dir");
        System.setProperty("codecli.audit.dir", tempDir.resolve("audit").toString());
        try {
            body.run();
        } finally {
            if (previous == null) {
                System.clearProperty("codecli.audit.dir");
            } else {
                System.setProperty("codecli.audit.dir", previous);
            }
        }
    }
