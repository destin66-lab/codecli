...
import com.codecli.render.inline.TerminalCapabilities;
 ...
 * <p>选型规则：
 * <ul>
 *   <li>{@code -Dcodecli.renderer} > {@code CODECLI_RENDERER} 环境变量 > 默认 inline</li>
 ...
 *   <li>兼容：{@code CODECLI_TUI=true} → 等价 {@code lanterna}，打 deprecation 提示</li>
 * </ul>
 *
...
public final class RendererFactory {
    ...
    public static Mode resolveMode() {
        ...
        // 兼容旧 CODECLI_TUI=true → lanterna
        String legacyTui = System.getenv("CODECLI_TUI");
        if (legacyTui != null && Boolean.parseBoolean(legacyTui.trim())) {
            System.err.println("⚠️ CODECLI_TUI=true 已废弃，请改用 CODECLI_RENDERER=lanterna");
    ...
    private static Mode parse(String raw) {
        return switch (raw.trim().toLowerCase()) {
            ...
            default -> {
                System.err.println("⚠️ 未识别的 CODECLI_RENDERER='" + raw + "'，回退到 inline");
                yield Mode.INLINE;
            }
        };
    }
