...
public class PlanExecuteAgent {
    ...
    public void setTraceSink(TraceSink sink) {
        this.traceSink = sink == null ? TraceRuntime.defaultSink() : sink;
    ...
    public String run(String userInput) {
        TraceSink.TraceHandle turn = traceSink.startTurn("agent.plan", userInput,
                Map.of("mode", "plan", "provider", llmClient.getProviderName(), "model", llmClient.getModelName()));
        try (TraceRuntime.Scope ignored = TraceRuntime.enter(new TraceRuntime.TraceContext(traceSink, turn, null))) {
            ...
            traceSink.score(turn.traceId(), "task_success", result == null || result.isBlank() ? 0 : 1,
                    "NUMERIC", Map.of("source", "plan.run"));
            traceSink.endTurn(turn, result == null || result.isBlank() ? "failed" : "completed", result, null);
            return result;
        } catch (RuntimeException e) {
            traceSink.endTurn(turn, "error", null, e);
            throw e;
        }
    }
...
/**
    ...
    private List<TaskExecutionResult> executeTaskBatch(ExecutionPlan plan, List<Task> executableTasks,
        ...
        ExecutorService executor = Executors.newFixedThreadPool(Math.min(executableTasks.size(), 4), r -> {
            Thread t = new Thread(r, "codecli-plan-executor");
            t.setDaemon(true);
            return t;
        });
        try {
