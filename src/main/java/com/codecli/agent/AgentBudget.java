 ...
 * 1. 系统属性：{@code codecli.react.token.budget} / {@code codecli.react.stagnation.window} /
 *    {@code codecli.react.hard.max.iterations}
 ...
 * 场景（CI / 自动化批跑）通过 {@code -Dcodecli.react.token.budget=N} 显式启用。
...
public class AgentBudget {
    ...
    public static AgentBudget fromLlmClient(LlmClient llmClient) {
        ...
        // 显式 -Dcodecli.react.token.budget=N 仍可启用硬预算，覆盖默认。
        return new AgentBudget(
                readIntProperty("codecli.react.token.budget", Integer.MAX_VALUE),
                readIntProperty("codecli.react.stagnation.window", DEFAULT_STAGNATION_WINDOW),
                readIntProperty("codecli.react.hard.max.iterations", DEFAULT_HARD_MAX_ITERATIONS)
        );
    }
