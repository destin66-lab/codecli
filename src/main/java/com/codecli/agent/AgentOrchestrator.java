...
public class AgentOrchestrator {
     ...
     */
    public void setSkillSystem(com.codecli.skill.SkillRegistry skillRegistry,
                               com.codecli.skill.SkillContextBuffer skillContextBuffer) {
    ...
    public String run(String userInput) {
        TraceSink.TraceHandle turn = traceSink.startTurn("agent.team", userInput,
                Map.of("mode", "team", "provider", llmClient.getProviderName(), "model", llmClient.getModelName()));
        try (TraceRuntime.Scope ignored = TraceRuntime.enter(new TraceRuntime.TraceContext(traceSink, turn, null))) {
            ...
            traceSink.score(turn.traceId(), "task_success", result == null || result.isBlank() ? 0 : 1,
                    "NUMERIC", Map.of("source", "team.run"));
    ...
    private void runBatchParallel(List<ExecutionStep> batch, List<ExecutionStep> steps,
        ...
        ExecutorService executor = Executors.newFixedThreadPool(parallelism, r -> {
            Thread t = new Thread(r, "codecli-multi-agent");
            t.setDaemon(true);
            return t;
        });
