package com.codecli.agent;

import com.codecli.llm.LlmClient;
import com.codecli.llm.LlmTraceLogger;
import com.codecli.context.ContextProfile;
import com.codecli.context.TokenUsageFormatter;
import com.codecli.lsp.LspDiagnosticReport;
import com.codecli.memory.ConversationHistoryCompactor;
import com.codecli.memory.ConversationLogger;
import com.codecli.memory.ExplicitMemoryHints;
import com.codecli.memory.MemoryManager;
import com.codecli.prompt.PromptAssembler;
import com.codecli.prompt.PromptContext;
import com.codecli.prompt.PromptMode;
import com.codecli.prompt.ProjectMemoryLoader;
import com.codecli.render.PlainRenderer;
import com.codecli.render.Renderer;
import com.codecli.render.StatusInfo;
import com.codecli.runtime.CancellationContext;
import com.codecli.skill.SkillContextBuffer;
import com.codecli.skill.SkillIndexFormatter;
import com.codecli.skill.SkillRegistry;
import com.codecli.util.AnsiStyle;
import com.codecli.tool.ToolRegistry;
import com.codecli.tool.ToolRegistry.ToolExecutionResult;
import com.codecli.tool.ToolRegistry.ToolInvocation;
import com.codecli.util.TerminalMarkdownRenderer;
import com.codecli.image.ImageReferenceParser;
...
public class Agent {
    ...
    private TraceSink traceSink = TraceRuntime.defaultSink();
    ...
    public Agent(LlmClient llmClient, ToolRegistry toolRegistry) {
        ...
        this.toolRegistry.setTraceSink(traceSink);
    ...
    public void setTraceSink(TraceSink traceSink) {
        this.traceSink = traceSink == null ? TraceRuntime.defaultSink() : traceSink;
        this.toolRegistry.setTraceSink(this.traceSink);
    }
    ...
    /**
     * 注入对话落盘器（测试可传入指向临时目录的实现；默认懒加载到 ~/.codecli/sessions）
     */
    public void setConversationLogger(ConversationLogger conversationLogger) {
        this.conversationLogger = conversationLogger;
    }

    private ConversationLogger conversationLogger() {
        if (conversationLogger == null) {
            conversationLogger = new ConversationLogger(toolRegistry.getProjectPath());
        }
    ...
    public String run(String userInput) {
        TraceSink.TraceHandle turn = traceSink.startTurn("agent.turn", userInput,
                java.util.Map.of("mode", "react", "provider", llmClient.getProviderName(),
                        "model", llmClient.getModelName()));
        try (TraceRuntime.Scope ignored = TraceRuntime.enter(
                new TraceRuntime.TraceContext(traceSink, turn, null))) {
            String result = runInternal(userInput);
            TraceSink.TraceObservation answer = traceSink.startObservation(turn, "final.answer", "span",
                    java.util.Map.of("mode", "react"));
            traceSink.endObservation(answer, "completed", result, null);
            traceSink.score(turn.traceId(), "task_success", isSuccessfulResult(result) ? 1 : 0,
                    "NUMERIC", java.util.Map.of("source", "agent.run"));
            traceSink.endTurn(turn, isSuccessfulResult(result) ? "completed" : "failed", result, null);
            return result;
        } catch (Throwable error) {
            traceSink.endTurn(turn, "error", null, error);
    ...
    private boolean isSuccessfulResult(String result) {
        ...
        return !normalized.contains("失败") && !normalized.contains("错误")
    ...
    private String runInternal(String userInput) {
        ...
        while (true) {
            ...
            conversationLogger().sync(conversationHistory);
            maybeCompactHistory();
            ...
            try {
                ...
                // 调用 LLM
                TraceSink.TraceObservation generation = traceSink.startObservation(
                        TraceRuntime.current() == null ? null : TraceRuntime.current().turn(),
                        "llm.generation", "generation",
                        java.util.Map.of("iteration", iteration, "provider", llmClient.getProviderName(),
                                "model", llmClient.getModelName(), "message_count", conversationHistory.size()));
                LlmClient.ChatResponse response;
                try {
                    response = llmClient.chat(conversationHistory, toolDefinitions, streamRenderer);
                    traceSink.endObservation(generation, "completed", java.util.Map.of(
                            "input_tokens", response.inputTokens(), "output_tokens", response.outputTokens(),
                            "cached_input_tokens", response.cachedInputTokens(),
                            "tool_call_count", response.toolCalls() == null ? 0 : response.toolCalls().size()), null);
                } catch (IOException error) {
                    traceSink.endObservation(generation, "error", null, error);
                    throw error;
                }
                ...
                if (response.hasToolCalls()) {
                    ...
                    TraceRuntime.Scope generationScope = TraceRuntime.enter(
                            TraceRuntime.current() == null ? null
                                    : TraceRuntime.current().withParent(generation));
    ...
    public String getContextStatus() {
        com.codecli.context.ContextProfile profile = memoryManager.getContextProfile();
        ...
        for (LlmClient.Message msg : conversationHistory) {
            int t = com.codecli.memory.TokenBudget.estimateMessagesTokens(java.util.List.of(msg));
            switch (msg.role()) {
    ...
    private int estimateToolsSchemaTokens() {
        try {
            return com.codecli.memory.MemoryEntry.estimateTokens(
    ...
    private long estimateCurrentContextTokens() {
        long messageTokens = com.codecli.memory.TokenBudget.estimateMessagesTokens(conversationHistory);
    ...
    private void logRequestContext(String scope, List<LlmClient.Tool> tools) {
        ...
        for (int messageIndex = 0; messageIndex < conversationHistory.size(); messageIndex++) {
            ...
            messages++;
            int tokens = com.codecli.memory.TokenBudget.estimateMessagesTokens(List.of(msg));
        ...
        if (tools != null && !tools.isEmpty()) {
            try {
                toolsSchemaTokens = com.codecli.memory.MemoryEntry.estimateTokens(
