...
public class ContextCompressor {
     ...
     *
     * @param memory          短期记忆
     * @param longTermMemory  非 null 时，压缩前把旧消息中的稳定事实提取并存入长期记忆（落盘）
     * @return 压缩后的摘要，如果不需要压缩则返回 null
     */
    public String compress(ConversationMemory memory, LongTermMemory longTermMemory) {
        ...
        // 上一轮摘要单独取出，不参与本轮 map 摘要（避免被当作普通历史重压导致信息衰减）
        ...
        // 压缩前先把旧消息里的稳定事实存入长期记忆，防止"压缩即永久丢失"
        if (longTermMemory != null) {
            extractFacts(oldEntries, longTermMemory);
        }
        ...
        // 增量合并：基于上一轮摘要更新，而不是从零重压，避免反复压缩逐级衰减
        if (previousSummary != null && !previousSummary.isBlank()) {
            finalSummary = updatePhase(finalSummary, previousSummary);
        }
    ...
    /**
     * Update 阶段：基于上一轮摘要增量合并，保留旧信息、补充新信息。
     * 失败时降级为"旧摘要 + 新摘要"拼接，至少不丢新内容。
     */
    private String updatePhase(String newSummary, String previousSummary) {
        try {
            String prompt = String.format(UPDATE_PROMPT, previousSummary, newSummary);
            List<LlmClient.Message> messages = List.of(
                    LlmClient.Message.system("你是一个摘要更新助手。"),
                    LlmClient.Message.user(prompt)
            );

            LlmClient.ChatResponse response = llmClient.chat(messages, null);
            String updated = response == null ? null : response.content();
