...
public abstract class AbstractOpenAiCompatibleClient implements LlmClient {
    ...
    protected static final OkHttpClient SHARED_HTTP_CLIENT = new OkHttpClient.Builder()
            .connectTimeout(readTimeoutSeconds("codecli.llm.connect.timeout.seconds", 60), TimeUnit.SECONDS)
            .readTimeout(readTimeoutSeconds("codecli.llm.read.timeout.seconds", 300), TimeUnit.SECONDS)
            .writeTimeout(readTimeoutSeconds("codecli.llm.write.timeout.seconds", 60), TimeUnit.SECONDS)
            .callTimeout(readTimeoutSeconds("codecli.llm.call.timeout.seconds", 600), TimeUnit.SECONDS)
            .build();
    ...
    /** Maximum completion budget after reserving headroom for the prompt. */
    protected int computeMaxOutputTokens(int inputTokens) {
        int available = maxContextWindow() - Math.max(0, inputTokens) - 256;
        return Math.max(256, Math.min(16_384, available));
    }
