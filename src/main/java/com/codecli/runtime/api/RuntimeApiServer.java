package com.codecli.runtime.api;

import com.fasterxml.jackson.databind.JsonNode;
import com.fasterxml.jackson.databind.ObjectMapper;
import com.codecli.runtime.task.TaskRunner;
...
public class RuntimeApiServer implements AutoCloseable {
    ...
    private final ExecutorService executor = Executors.newCachedThreadPool(r -> {
        Thread thread = new Thread(r, "codecli-runtime-api");
    ...
    public RuntimeApiServer(RuntimeThreadStore store, TaskRunner runner, int port, String apiKey) throws IOException {
        if (apiKey == null || apiKey.isBlank()) {
            throw new IllegalArgumentException("Runtime API 需要配置 CODECLI_RUNTIME_API_KEY 或 -Dcodecli.runtime.api.key");
        }
        this.store = store;
        this.runner = runner;
        this.apiKey = apiKey;
    ...
    public static String configuredApiKey() {
        String configured = firstNonBlank(
                System.getProperty("codecli.runtime.api.key"),
                System.getenv("CODECLI_RUNTIME_API_KEY"));
        return configured;
    }
    ...
    private boolean authorized(HttpExchange exchange) {
        ...
        String direct = exchange.getRequestHeaders().getFirst("X-CodeCLI-API-Key");
